...
class HistogramHeuristicGenerator(object):
    ...
    def set_deg_data(self, degrees, features):
        ...
        self._features = np.hstack([np.array(degrees)[:, np.newaxis], features])
    ...
    def _check_data_(self):
        if np.min(self._features) <= 0.0:
            print("Error: The input feature contains non-postive values, "
                  "which can't be applied with logatithm")
            exit()
    ...
    def save_histogram(self, outfn, sep=','):
        with open(outfn, 'w') as ofp:
            header = "# {}".format(str(self.shape)[1:-1].replace(', ', sep))
    ...
    def save_pts_index(self, outfn, sep=',', pts_idx=None):
        header = "#pts: {}; ranges: {}".format(len(self._features), str(self.shape)[1:-1].replace(', ', sep))
    ...
    def save_hpos2avgfeat(self, outfn, sep=','):
        with open(outfn, 'w') as ofp:
            pos_xs, feat_vs = "", ""
